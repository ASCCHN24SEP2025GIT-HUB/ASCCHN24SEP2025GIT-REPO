...
SELECT Name, ListPrice, RANK() OVER (ORDER BY ListPrice DESC) AS PriceRank 
FROM SalesLT.Product;
...
SELECT ROW_NUMBER() OVER (ORDER BY LastName, FirstName) AS RowNum, CustomerID, FirstName, LastName 
FROM SalesLT.Customer;
...
SELECT p.Name, pc.Name AS Category, p.ListPrice, 
       DENSE_RANK() OVER (PARTITION BY p.ProductCategoryID ORDER BY p.ListPrice DESC) AS CategoryRank 
FROM SalesLT.Product p
INNER JOIN SalesLT.ProductCategory pc ON p.ProductCategoryID = pc.ProductCategoryID;
...
SELECT FORMAT(c.ModifiedDate, 'yyyy-MM') AS CohortMonth,
       COUNT(DISTINCT c.CustomerID) AS CustomersInCohort,
       COUNT(soh.SalesOrderID) AS TotalOrders,
       SUM(soh.TotalDue) AS TotalRevenue 
FROM SalesLT.Customer c
LEFT JOIN SalesLT.SalesOrderHeader soh ON c.CustomerID = soh.CustomerID 
GROUP BY FORMAT(c.ModifiedDate, 'yyyy-MM') 
...
SELECT pc1.Name AS ParentCategory, pc2.Name AS ChildCategory,
...
LEFT JOIN SalesLT.Product p ON pc2.ProductCategoryID = p.ProductCategoryID
LEFT JOIN SalesLT.SalesOrderDetail sod ON p.ProductID = sod.ProductID 
WHERE pc1.ParentProductCategoryID IS NULL 
GROUP BY pc1.Name, pc2.Name 
ORDER BY pc1.Name, TotalSales DESC;
...
SELECT c.CustomerID, c.FirstName, c.LastName,
       ...
       DATEDIFF(MONTH, MIN(soh.OrderDate), MAX(soh.OrderDate)) AS CustomerTenureMonths,
       CASE 
           WHEN DATEDIFF(MONTH, MIN(soh.OrderDate), MAX(soh.OrderDate)) > 0 
           THEN SUM(soh.TotalDue) / DATEDIFF(MONTH, MIN(soh.OrderDate), MAX(soh.OrderDate)) 
